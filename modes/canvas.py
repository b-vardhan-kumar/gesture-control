...
PALETTE_BOX_W  = 140  # increased width for bigger window
HEADER_HEIGHT = 100    # moved palette down
HOVER_SELECT_SECONDS = 0.5
BRUSH_THICKNESS = 12   # increased brush thickness
...
def map_fingertip_to_palette(x, y):
    if HEADER_HEIGHT <= y <= (HEADER_HEIGHT + 100):  # increased height
        idx = x // PALETTE_BOX_W
...
def run(frame, canvas_frame, results, last_action):
    ...
    if results.multi_hand_landmarks:
        ...
        # Draw pointer
        cv2.circle(frame, (ix, iy), 10, (0,120,255), -1)
    ...
    for i, (col, name) in enumerate(PALETTE_COLS):
        ...
        if col == "CLEAR":
            ...
            cv2.putText(frame,"CLEAR",(x1+20,y1+65),cv2.FONT_HERSHEY_SIMPLEX,1,(200,200,200),2)
        else:
            cv2.rectangle(frame,(x1,y1),(x2,y2),col,-1)
            cv2.putText(frame,name,(x1+20,y2-20),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,0,0),2)
